...
function bindDeleteClick() {
    $('.deleteComment').off().on('click',function () {
        ...
        // 异步删除
        $.ajax({
            url : '/timelineComment/deleteComment',
            method : 'post',
            dataType : 'json',
            data : {"id" : id},
    ...
    $('.pl-sub').click(function(){
        ...
        if(comment == ''){
            ...
            if($('#userIdHead').val() == undefined || $('#userIdHead').val() == ''){
                ...
                // 提交评论
                $.ajax({
                    url : '/timelineComment/saveComment',
                    method : 'post',
                    data : {"creator" : $('#userIdHead').val(),"pid" : pid,"pidType":pidType,"comment" : comment,"timelineInfoId": $('#timelineInfoId').val()},
                    dataType : 'json',
                    success : function (data) {
                        if(data.status == 'S'){
